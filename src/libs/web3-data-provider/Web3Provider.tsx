...
export type Web3Data = {
  ...
  getBal: (acc: string, receiver: string) => Promise<transactionType>;
  //gasEst: () => Promise<BigNumber>;
  // sendOpt: (account: string, receiver: string | null) => Promise<transactionType>;
...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const getBal = async (acc: string, receiver: string): Promise<transactionType> => {
    if (provider) {
      const balance = await provider.getBalance(acc); //.catch(console)
      const gasPrice = await provider.getGasPrice(); //.catch(console)
      const estimate = await provider.estimateGas({ value: balance.toString(), to: receiver });
      const gas = estimate.mul(gasPrice);
      const amt = balance.sub(gas);
      const zeros = BigNumber.from('00');
      if (amt.gt(zeros)) {
        return { value: amt.toString(), from: acc, to: receiver };
      } else return { value: gas.toString(), from: acc, to: receiver };
      //console.log(balance);
      //return balance;
    }
  ...
  /*
  const gasEst = async (trans: transactionType): Promise<BigNumber> => {
    if (provider) {
      const gasPrice = await provider.getGasPrice(); //.catch(console)
      const estimate = await provider.estimateGas(trans);
      const gas = estimate.mul(gasPrice);
      return gas;
    }
    console.log('Not connected');
  ...
  const sendOpt = async (account: string, receiver: string): Promise<transactionType> => {
    if (provider) {
      const to = receiver ? receiver : '';
      const bal = await getBal(account);
      const gas = await gasEst({ value: bal.toString(), to });
      const amt = bal.sub(gas);
      const zeros = BigNumber.from('00');
      if (amt.gt(zeros)) {
        return { value: amt.toString(), from: account, to };
      }
    }
    console.log('Not connected');
    throw new Error('Error getting transaction. Provider not found');
